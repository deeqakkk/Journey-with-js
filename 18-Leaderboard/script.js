...
button.addEventListener("click", () => {
  if (!(first_Name.value) || !(last_Name.value) || !(country.value) ||
      !(score.value)) {
    ...
    console.log(months[date.getMonth()] + " " + date.getDay() + " " +
                date.getFullYear() + " " + ('0' + date.getHours()).slice(0, 2) +
                ":" + ('0' + date.getMinutes()).slice(0, 2));
    let div2 = document.createElement('span');
    div2.innerHTML = months[date.getMonth()] + " " + date.getDay() + " " +
                     date.getFullYear() + " " +
                     ('0' + date.getHours()).slice(0, 2) + ":" +
                     ('0' + date.getMinutes()).slice(0, 2);
    ...
    add5_button.addEventListener('click', () => {
      ...
      let elements =
          document.querySelector('.' + classadd5 + '').children.item(2);
    ...
    subs5_button.addEventListener('click', () => {
      ...
      let elements =
          document.querySelector('.' + classsubs5 + '').children.item(2);
